...
xminc, xmaxc, zminc = -35e3, 35e3, -35e3
...
# xi, minimum Dc
xi              = 0.005 # 0.02 for planar; xi used to limit variable time step size. See Lapusta et al. (2009).
...
on_fault_vars = np.zeros((par.ntotft, nfz, nfx, 100))
...
for ix, xcoor in enumerate(fx):
  ...
  # assign a in RSF. a is a 2D distribution.
    ...
    on_fault_vars[0,iz,ix,9]  = fric_rsf_a + (1. - tmp1*tmp2)*fric_rsf_deltaa
...
####################################
casename       = "liu2020.fdc.rough.250.quasi"
