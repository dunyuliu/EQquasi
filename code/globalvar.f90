MODULE globalvar
	...
	real (kind = dp)::term=200.0d0,dt=0.05d0
	integer(kind=4)::friclaw=3
	real (kind = dp)::fltxyz(2,4,1)
	real (kind = dp)::rdalfa=0.00d0,rdbeta=0.0d-3, slipr_thres = 1.0d-3
	integer(kind=4)::nplpts, nhplt=20, nstep=1000000
	real (kind = dp)::pi=3.14159265358979323846d0,time=0.0d0,critt0=3.0d3,k0=1.0d8
	real(kind = 8) :: ksi=0.025d0, LL=0.13d0, ttheta=10.0d0, loadrate = 5.0d-10, pma, sliprmax
	integer(kind=4)::OMPNUM=20,epsi = 1, C_farfield = 0, icstart
