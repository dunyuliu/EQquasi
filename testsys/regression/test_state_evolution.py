...
def test_plot_scripts_do_not_hardcode_the_txt_station_extension():
    ...
    script/plotDomainSweep.py looked for fltst_strk+000dp+000.txt, which no
    ...
    bad = []
    for path in _glob.glob(os.path.join(str(ROOT), "script", "*.py")):
        src = read(os.path.join("script", os.path.basename(path)))
