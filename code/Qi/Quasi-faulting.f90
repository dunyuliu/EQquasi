...
enddo	!ending i
	if (((status1==0.and.itag==1).and.(mod(it,20)==1)).or.((status1==1.and.itag==1).and.(mod(it,10)==0))) then 
	        open(9002,file='p1output.txt',form='formatted',status='unknown',position='append')
				do i = 1,nftnd
					if (abs(x(1,nsmp(1,i)))<=37.0d3.and.abs(x(3,nsmp(1,i))--60.0d3)<0.01d0) then
						write(9002,'(1x,5e32.21e4)') time,fltslp(1,i),fltslp(2,i),fric(28,i),fric(29,i)
					endif
				enddo
	        open(9003,file='p2output.txt',form='formatted',status='unknown',position='append')
				do i = 1,nftnd
					if (abs(x(1,nsmp(1,i)))<=0.01d0.and.abs(x(3,nsmp(1,i))--60.0d3)<=40.0d3) then
						write(9003,'(1x,5e32.21e4)') time,fltslp(1,i),fltslp(2,i),fric(28,i),fric(29,i)
					endif
				enddo				
        endif
	if (itag==1) then
