...
1. model.txt is positional and unlabelled: script/case.setup writes N lines
...
def model_txt_write_count():
    """Number of lines script/case.setup writes into model.txt."""
...
def test_model_txt_write_and_read_counts_agree():
    ...
    assert w == r, (
        f"script/case.setup writes {w} model.txt lines but "
...
def test_netcdf_on_fault_variable_names_agree_between_writer_and_reader():
    ...
    py = strip_python_comments(read("script/case.setup"))
...
def test_benchmark_file_parsing_is_not_reimplemented_per_script():
    """One parser for benchmark output files, in script/seasio.py.
    ...
    offenders = []
    for path in glob.glob(os.path.join(str(ROOT), "script", "*")):
