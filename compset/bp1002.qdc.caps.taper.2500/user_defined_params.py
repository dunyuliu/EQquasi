#! /usr/bin/env python3
# compset: bp1002.qdc.caps.taper.2500
# gate   : none -- UNVERIFIED (new experiment, no reference; board row 16)
# changed: 2026-09-25
# note   : bp1002.qdc.caps.2500 plus a 5 km velocity-weakening -> velocity-
#          strengthening taper at each segment's INTERIOR tip (the ends that
#          face each other across the step-over). Everything else -- caps,
#          geometry, outer taper, z-bands, nucleation patch -- is the sibling's.
...
+x end at +2.5 km, segment B's -x end at -2.5 km) are TAPERED here, which is
the one thing this compset changes relative to bp1002.qdc.caps.2500. Every
earlier bp1002 compset left them fully velocity-weakening (an idealization
the owner ruled acceptable at the time); in the uncapped run that unclamped
fault A's tip at the free surface by cycle 9 (STOP 508), and in the capped
run scratch/bp1002caps2.sci the effective normal stress in the 5 km overlap
sat pinned at the -10 MPa cap for 25 cycles with 16 A-only / 14 B-only /
0 joint ruptures. Tapering is the physical fix the caps only paper over.

Inner-tip taper, per segment, measured from that segment's own inner tip:
  - d_tip = 0      velocity-strengthening (a + deltaa), the tip node itself
  - 0 < d_tip < 5  linear ramp, a + (1 - d_tip/5 km) * deltaa
  - d_tip >= 5 km  velocity-weakening (subject to the z bands)
Why 5 km and not the outer taper's 2 km: at dx = 2500 m a 2 km ramp holds
no node -- it would flip the tip node to VS and leave its neighbour fully
VW, a step, not a taper. 5 km is two elements (tip VS, next node a + 0.5
deltaa, third node VW), the shortest ramp this mesh can resolve, and it is
exactly the 5 km overlap length, so the tapered region coincides with the
region where sigma_bar pinned at the cap. The nucleation patch (x in
[-36, -24] km on A) is 21 km from A's inner tip and untouched.
...
# Inner-tip taper length (m): the segment end nearest the model centre is
# the inner tip; a ramps from a + deltaa there to a at TIP_TAPER inboard.
TIP_TAPER = 5.0e3

for ift, (xlo, xhi, ycoor, zlo, zhi) in enumerate(par.faultgeom):
    init_norm = init_norm_by_fault[ift]
    inner_tip_x = xlo if abs(xlo) < abs(xhi) else xhi
    ...
    for ix, xcoor in enumerate(fx):
        for iz, zcoor in enumerate(fz):
            ...
            #
            # The step-over sits at x = 0. Unlike the sibling compset the
            # interior tips facing each other are tapered VW -> VS over
            # TIP_TAPER from each segment's own inner tip (see docstring).
            dtip = abs(xcoor - inner_tip_x)
            if abs(zcoor) >= 18e3 or abs(zcoor) <= 2e3 or abs(xcoor) >= 40e3:
                a = par.fric_rsf_a + par.fric_rsf_deltaa  # creeping (VS)
                    ...
                    and dtip >= TIP_TAPER:
                a = par.fric_rsf_a  # locked (VW)
            else:
                tmp1 = (abs(abs(zcoor) - 10e3) - 6e3) / 2e3
                tmp2 = (abs(xcoor) - 38e3) / 2e3
                tmp3 = (TIP_TAPER - dtip) / TIP_TAPER  # 1 at the tip, 0 at 5 km
                a = par.fric_rsf_a + max(tmp1, tmp2, tmp3) * par.fric_rsf_deltaa
...
####################################
par.casename = "bp1002-stepover-qd-caps-taper-2500"
par.HPC_nnode = 1
par.HPC_ncpu = 3
