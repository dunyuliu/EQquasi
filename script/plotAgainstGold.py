...
`testsys/e2e` tells you *whether* a run still matches its oracle. When it does not,
...
def compare_field(bench, run_dir, out, only=None):
    ...
    # script reported "matches gold" while `testsys/e2e/test_benchmarks.py`'s xarray identical()
