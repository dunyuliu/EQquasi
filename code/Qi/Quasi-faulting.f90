...
integer (kind = 4), dimension(nftnd) :: sliprate_increase_arr, sliprate_increase_VW_arr
...
sliprate_increase_arr = 0
sliprate_increase_VW_arr = 0
...
!---3.4.1.1: DYNAMIC PROCESS, [STATUS1]==1		
			...
			!endif		 
			if((tnrm+fric(6,i))>normalcap) then
				tnrm0 = normalcap
			else
				tnrm0 = tnrm+fric(6,i)
			endif	
...
!----------: THETA**(t+1) [FRIC(22,:)] FOR ITAG==1, , USING [V_TRIAL]={[CONSTRAINV]+[CONSTRAINVTMP]}*0.5	
			...
			!		fric(22,i) = fric(11,i)/fric(12,i)*dexp(fric(22,i))
			!	endif
                fric(22,i) = fric(11,i)/v_trial + (fric(20,i) - fric(11,i)/v_trial) * dexp(-v_trial * dtev1 / fric(11,i))
			elseif (itag == 1) then 
				do j=1,2  !1-slave, 2-master
					do k=1,3  !1-x comp, 2-y comp, 3-z comp 
			...
			tnrm = (mslav * fvd(4,2,1) - mmast * fvd(4,1,1)) / mtotl + fnfault
			!tnrm = (mslav*mmast*((fvd(4,2,2)-fvd(4,1,2))+(fvd(4,2,3)-fvd(4,1,3))/dtev1)/dtev1 &
			!	+ mslav*fvd(4,2,1) - mmast*fvd(4,1,1)) / mtotl + fnfault         
			!tstk0 = (mslav*mmast*(fvd(5,2,2)-fvd(5,1,2))/dtev1 + mslav*fvd(5,2,1) &
			!	- mmast*fvd(5,1,1)) / mtotl + fsfault
			!tdip0 = (mslav*mmast*(fvd(6,2,2)-fvd(6,1,2))/dtev1 + mslav*fvd(6,2,1) &
			!	- mmast*fvd(6,1,1)) / mtotl + fdfault            
			! fric(41,i) is abs(KU)
...
!---3.4.1.4: NEWTON METHOD TO GET NEW V_TRIAL BASED ON TRACTION=ELASTIC FORCE.			
            ...
            ivmax = 1000 
			 do iv = 1,ivmax	
				...
				! drsfeqdv = 1.0d0 + T_coeff * (-dxmudv * MIN(tnrm0,0.0d0))*0.5d0				
				...
				if (x(1,isn)==6d3.and.x(3,isn)==-12d3)then
					write(*,*) 'FAULTING=======AAA======ITAG=',itag
					write(*,*) 'iv,vtrial,phi*',iv,v_trial,fric(22,i)
					!write(*,*) 'fvd,mast,slav',fvd(5,2,1),fvd(5,1,1)
					write(*,*) 'tau_fric_trial,ttao0', tau_fric_trial
					!write(*,*) 'tstk0,tdip0',tstk0/1.0d6,tdip0/1.0e6
				endif	
...
!---3.4.1.5: V* FOR ITAG==0 AND V** FOR ITAG==1 OBTAINED. 			
			...
			elseif (itag == 1) then 
				fric(20,i) = fric(22,i)
				if (v_trial > fric(42,i)) then
						sliprate_increase_arr(i) = 1
				endif
				if (v_trial > 0.003d0 .and. abs(x(1,isn)) < 22.0d3 .and. abs(x(3,isn))< 18.0d3) then
						sliprate_increase_VW_arr(i) = 1
				endif
...
enddo	!ending i
	...
	if ((status1==0.and.itag==1).or.(status1==1)) then
                ...
                sliprate_increase_count = sum(sliprate_increase_arr)
                sliprate_increase_count_VW = sum(sliprate_increase_VW_arr)	
