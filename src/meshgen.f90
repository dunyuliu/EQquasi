            ...
            do iy = 1, nyt
                ...
                do ift=1,ntotft 
                    ...
                    !For TPV8    
                        ...
                        fric(FR_PORE_DP,nftnd0(ift),ift) = 0.0d0!rhow*grav*(-zcoor) !pore pressure
                        ! -------------------------------------------------------------------------
                        ! Case specific setup
                        ! if (friclaw == 3 .and. bp == 5) then
                            ! ! Setting a, b, Dc, v0, r0
                            ! if(abs(zcoor)>=18.0d3.or.abs(xcoor)>=32.0d3.or.abs(zcoor)<=2.0d3) then
                                ! fric(FR_RSF_A,nftnd0(ift),ift) = fric_rsf_a + fric_rsf_deltaa0
                            ! elseif (abs(zcoor)<=16.0d3.and.abs(zcoor)>=4.0d3.and.abs(xcoor)<=30.0d3) then
                                ! fric(FR_RSF_A,nftnd0(ift),ift) = fric_rsf_a
                            ! else
                                ! tmp1 = abs(abs(zcoor)-2.0d3-2.0d3-12.0d3/2.0d0) - 12.0d3/2.0d0
                                ! tmp1 = tmp1 / 2.0d3
                                ! tmp2 = (abs(xcoor)-30.0d3)/2.0d3        
                                ! fric(FR_RSF_A,nftnd0(ift),ift) = fric_rsf_a + max(tmp1,tmp2)*fric_rsf_deltaa0                     
                            ! endif    
                            ! fric(FR_RSF_B,nftnd0(ift),ift) = fric_rsf_b                                            
                            ! fric(FR_RSF_DC,nftnd0(ift),ift) = fric_rsf_Dc !RSF critical distance.
                            ! if (xcoor<-30.0d3+12.0d3+tol.and.xcoor>-30.0d3-tol.and.zcoor<-4.0d3+tol &
                                 ! .and.zcoor>-16.0d3-tol) then
                                ! fric(FR_RSF_DC,nftnd0(ift),ift) = minDc
                            ! endif 
                            ! fric(FR_RSF_V0,nftnd0(ift),ift) = fric_rsf_v0 !RSF:V0
                            ! fric(FR_RSF_F0,nftnd0(ift),ift) = fric_rsf_r0 !RSF:miu0

                            ! fric(FR_VINI_N,nftnd0(ift),ift) = 0.0d0 !RSF: initial normal slip rate 
                            ! fric(FR_VINI_S,nftnd0(ift),ift) = fric_rsf_vinix !RSF:s 
                            ! fric(FR_VINI_D,nftnd0(ift),ift) = fric_rsf_viniz !RSF:d
                            ! fric(FR_VINI_MAG,nftnd0(ift),ift) = sqrt(fric_rsf_vinix**2+fric_rsf_viniz**2) !RSF:mag        
                            ! ! Setting theta
                            ! if (icstart == 1) then 
                                ! fric(FR_TNRM0,nftnd0(ift),ift) = init_norm !-25.0d6 ! Initial normal stress 
                                ! fric(FR_STATE,nftnd0(ift),ift )= fric(FR_RSF_DC,nftnd0(ift),ift)/load_slip_rate ! theta0 for steady state at v==load_slip_rate.
                                ! ! Setting initial shear stress. 
                                ! !fric(FR_TSTK0,nftnd0(ift),ift) = - fric(FR_TNRM0,nftnd0(ift),ift) * fric(FR_RSF_A,nftnd0(ift),ift) *dasinh( &
                                ! !    1.0d-9/2.0d-6*dexp((0.6d0+0.03d0*dlog(1.0d-6/1.0d-9))/fric(FR_RSF_A,nftnd0(ift),ift))) + 2670.0d0*3464.0d0/2.0d0*1.0d-9
                                ! fric(FR_SPARE45, nftnd0(ift), ift) = 0.0d0
                                ! fric(FR_VINIT, nftnd0(ift), ift) = load_slip_rate ! initial slip rate.
                                ! call rsf_rd(fric(FR_TSTK0,nftnd0(ift),ift), fric(FR_TNRM0,nftnd0(ift),ift), fric(FR_RSF_A,nftnd0(ift),ift), fric(FR_RSF_B,nftnd0(ift),ift), fric(FR_RSF_F0,nftnd0(ift),ift), fric(FR_RSF_V0,nftnd0(ift),ift), mat0(1,2), mat0(1,3), fric(FR_VINIT, nftnd0(ift), ift))
                                ! fric(FR_SPARE44, nftnd0(ift), ift) = fric(FR_TSTK0, nftnd0(ift), ift) !tstk0
                                ! if (xcoor<-30.0d3+12.0d3+tol.and.xcoor>-30.0d3-tol.and.zcoor<-4.0d3+tol &
                                     ! .and.zcoor>-16.0d3-tol) then
                                    ! fric(FR_VINIT,nftnd0(ift),ift)=3.0d-2 ! In the special patch, increase initial slip rate. 
                                    ! fric(FR_TSTK0,nftnd0(ift),ift) = - fric(FR_TNRM0,nftnd0(ift),ift) * fric(FR_RSF_A,nftnd0(ift),ift) *dasinh( &
                                        ! fric(FR_VINIT,nftnd0(ift),ift)/2.0d-6*dexp((0.6d0+0.03d0*dlog(1.0d-6/1.0d-9))/fric(FR_RSF_A,nftnd0(ift),ift))) + 2670.0d0*3464.0d0/2.0d0*fric(FR_VINIT,nftnd0(ift),ift)
                                    ! !call rsf_rd(fric(FR_TSTK0,nftnd0(ift),ift), fric(FR_TNRM0,nftnd0(ift),ift), fric(FR_RSF_A,nftnd0(ift),ift), fric(FR_RSF_B,nftnd0(ift),ift), fric(FR_RSF_F0,nftnd0(ift),ift), fric(FR_RSF_V0,nftnd0(ift),ift), mat0(1,2), mat0(1,3), fric(FR_VINIT, nftnd0(ift), ift))
                                    ! fric(FR_SPARE44, nftnd0(ift), ift) = fric(FR_TSTK0, nftnd0(ift), ift)
                                    ! fric(FR_SPARE45, nftnd0(ift), ift) = 0.0d0
                                ! endif
                                ! fric(FR_V_CURRENT,nftnd0(ift),ift) = fric(FR_VINIT,nftnd0(ift),ift)! Peak slip rate
                            ! elseif (icstart > 1) then
                                ...
                                ! nfz = (zcoor - zmin)/dx+1
                                ! ntmp = (zmax-zmin)/dx + 1
                                ! fric(FR_STATE,nftnd0(ift),ift) = initial(4,(nfx-1)*ntmp +nfz) !theta0
                                ! fric(FR_TNRM0,nftnd0(ift),ift) = initial(7,(nfx-1)*ntmp +nfz) !tnrm0
                                ! fric(FR_TSTK0,nftnd0(ift),ift) = initial(5,(nfx-1)*ntmp +nfz) !tstk
                                ! fric(FR_TDIP0,nftnd0(ift),ift) = initial(6,(nfx-1)*ntmp +nfz) !tdip
                                ! fric(FR_VXM,nftnd0(ift),ift) = initial(8,(nfx-1)*ntmp +nfz) ! vx0+
                                ! fric(FR_VYM,nftnd0(ift),ift) = initial(9,(nfx-1)*ntmp +nfz) ! vy0+
                                ! fric(FR_VZM,nftnd0(ift),ift) = initial(10,(nfx-1)*ntmp +nfz) ! vz0+
                                ! fric(FR_VXS,nftnd0(ift),ift) = initial(11,(nfx-1)*ntmp +nfz) ! vx0-
                                ! fric(FR_VYS,nftnd0(ift),ift) = initial(12,(nfx-1)*ntmp +nfz)
                                ! fric(FR_VZS,nftnd0(ift),ift) = initial(13,(nfx-1)*ntmp +nfz)
                                ! fric(FR_V_CURRENT,nftnd0(ift),ift) = initial(3,(nfx-1)*ntmp +nfz)! Peak slip rate
                                ! fric(FR_SPARE44, nftnd0(ift), ift) = fric(FR_TSTK0, nftnd0(ift), ift) !tstk0
                                ! fric(FR_SPARE45, nftnd0(ift), ift) = 0.0d0
                             ! endif
                        ! endif !end case bp==5.
                        ! if (friclaw == 3 .and. bp == 1001) then
                            ...
                            ! if (icstart == 1) then 
                                ...
                                ! ! if (xcoor<-8.0d3+tol.and.xcoor>-20.0d3-tol.and.zcoor<-4.0d3+tol &
                                    ...
                                    ! ! fric(FR_VINIT,nftnd0(ift),ift)=3.0d-2 ! In the special patch, increase initial slip rate. 
                                    ! ! fric(FR_TSTK0,nftnd0(ift),ift) = - fric(FR_TNRM0,nftnd0(ift),ift) * fric(FR_RSF_A,nftnd0(ift),ift) *dasinh( &
                                        ! ! fric(FR_VINIT,nftnd0(ift),ift)/2.0d-6*dexp((0.6d0+0.03d0*dlog(1.0d-6/1.0d-9))/fric(FR_RSF_A,nftnd0(ift),ift))) + mat0(1,2)*mat0(1,3)/2.0d0*fric(FR_VINIT,nftnd0(ift),ift)
                                    ! ! !call rsf_rd(fric(FR_TSTK0,nftnd0(ift),ift), fric(FR_TNRM0,nftnd0(ift),ift), fric(FR_RSF_A,nftnd0(ift),ift), fric(FR_RSF_B,nftnd0(ift),ift), fric(FR_RSF_F0,nftnd0(ift),ift), fric(FR_RSF_V0,nftnd0(ift),ift), mat0(1,2), mat0(1,3), fric(FR_VINIT, nftnd0(ift), ift))
                                    ! ! fric(FR_SPARE44, nftnd0(ift), ift) = fric(FR_TSTK0, nftnd0(ift), ift)
                                    ! ! fric(FR_SPARE45, nftnd0(ift), ift) = 0.0d0
                                ! ! endif
